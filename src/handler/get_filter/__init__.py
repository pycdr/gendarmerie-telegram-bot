...
def start_process(update: Update, context: CallbackContext, model, token: str) -> int:
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton(
            text=group.name,
            callback_data="0103"+str(group.id)
        )]
        for group in model.Group.select()
        if is_admin(group.id, update.message.from_user.id, token)
    ])
    update.message.reply_text(
        "OK! choose one of your groups to get a filter (if there's any! :D)",
...
def state_get_group_by_callback(update: Update, context: CallbackContext, model, token: str) -> int:
    ...
    if not is_admin(group_id, query.from_user.id, token):
        ...
        return ConversationHandler.END
    query.answer()
    context.user_data["0103group_id"] = group_id
    query.edit_message_text(
        f"OK! for group \"{model.Group.get(model.Group.id == group_id).name}\", select one of these filter types:",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("remove message", callback_data="0103"+"0")],
            [
                InlineKeyboardButton("restrict user", callback_data="0103"+"1"),
                InlineKeyboardButton("ban user", callback_data="0103"+"2")
            ],
            [InlineKeyboardButton("back to the previous menu", callback_data="0103"+'GET_BACK')]
            ])
        )
    return GET_TYPE
...
def state_get_type_by_callback(update: Update, context: CallbackContext, model, token: str) -> int:
    ...
    if query.data[4:] != "GET_BACK":
        ...
        if type_id not in range(3):
            ...
            return ConversationHandler.END
        query.answer()
        context.user_data["0103type_id"] = type_id
        type_as_str = ["remove message mode", "restrict user mode", "ban user mode"][type_id]
        query.edit_message_text(
            f"OK! for {type_as_str}, choose one of this regexes:",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton(msg_filter.regex, callback_data="0103"+msg_filter.regex)]
                for msg_filter in model.Group.get(
                    model.Group.id == context.user_data["0103group_id"]
                ).filters.select().where(
                    model.Filter.type_id == type_id
                )
            ] + [[InlineKeyboardButton("get back to the previous menu", callback_data="0103"+"GET_BACK")]])
        )
        return GET_REGEX
    else:
        keyboard = InlineKeyboardMarkup([
...
def state_get_regex_by_callback(update: Update, context: CallbackContext, model, token: str) -> int:
    query = update.callback_query
    type_id = context.user_data["0103type_id"]
    if query.data[4:] != "GET_BACK":
        regex = query.data[4:]
        group_id = context.user_data["0103group_id"]
        command = next((msg_filter for msg_filter in model.Filter.select().where(
            model.Filter.type_id == type_id
        ...
        query.edit_message_text(
            ...
            f"+ regex: {regex}",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("back to the commands", callback_data="0103"+'GET_BACK')]])
        )
        return GET_BACK
    else:
        query.answer()
        query.edit_message_text(
            f"OK! for group \"{model.Group.get(model.Group.id == context.user_data['0103group_id']).name}\", select one of these filter types:",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("remove message", callback_data="0103"+"0")],
                [
                    InlineKeyboardButton("restrict user", callback_data="0103"+"1"),
...
def creator(model, token):
    get_special_handler = ConversationHandler(
        entry_points=[
            CommandHandler("getfilter", pass_model_and_token(start_process, model, token))
        ],
        states={
            GET_GROUP: [
                CallbackQueryHandler(
