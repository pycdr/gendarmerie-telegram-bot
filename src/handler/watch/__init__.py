...
def state_get_command_by_callback(update: Update, context: CallbackContext, model, token: str) -> int:
    ...
    if query.data[4:] != "GET_BACK":
        ...
        query.edit_message_text(
            ...
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("back to the commands", callback_data="0102"+'GET_BACK')]])
        )
        return GET_BACK
    else:
        keyboard = InlineKeyboardMarkup([
...
def creator(model, token):
    get_command_handler = ConversationHandler(
        entry_points=[
            CommandHandler("watch", pass_model_and_token(start_process, model, token))
        ],
        states={
            GET_GROUP: [
                CallbackQueryHandler(
